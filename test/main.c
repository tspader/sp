...
UTEST(sp_os_copy, copy_single_file) {
  sp_str_t test_dir = SP_LIT("build/test/sp_os_copy_single_file");
  sp_os_remove_directory(test_dir);
  sp_os_create_directory(test_dir);

  sp_str_t src = sp_os_join_path(test_dir, SP_LIT("source.txt"));
  sp_str_t dst = sp_os_join_path(test_dir, SP_LIT("dest.txt"));

  sp_io_stream_t stream = sp_io_from_file(src, SP_IO_MODE_WRITE);
  sp_str_t content = SP_LIT("test content");
  sp_io_write(&stream, content.data, content.len);
  sp_io_close(&stream);
...
UTEST(sp_os_copy, copy_file_to_directory) {
  sp_str_t test_dir = SP_LIT("build/test/sp_os_copy_file_to_dir");
  sp_os_remove_directory(test_dir);
  sp_os_create_directory(test_dir);

  sp_str_t src = sp_os_join_path(test_dir, SP_LIT("file.txt"));
  sp_str_t dst_dir = sp_os_join_path(test_dir, SP_LIT("dest"));
...
UTEST(sp_os_copy, copy_glob_files) {
  sp_str_t test_dir = SP_LIT("build/test/sp_os_copy_glob");
  sp_os_remove_directory(test_dir);
  sp_os_create_directory(test_dir);

  sp_str_t src_dir = sp_os_join_path(test_dir, SP_LIT("src"));
  sp_str_t dst_dir = sp_os_join_path(test_dir, SP_LIT("dst"));
...
UTEST(sp_os_copy, copy_directory) {
  sp_str_t test_dir = SP_LIT("build/test/sp_os_copy_directory");
  sp_os_remove_directory(test_dir);
  sp_os_create_directory(test_dir);

  sp_str_t src_dir = sp_os_join_path(test_dir, SP_LIT("source"));
  sp_str_t dst_dir = sp_os_join_path(test_dir, SP_LIT("dest"));
  sp_os_create_directory(src_dir);
  sp_os_create_directory(dst_dir);
