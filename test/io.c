...
UTEST_F(io_rw, reader_mem_read_full) {
  ...
  u64 bytes = sp_io_reader_read(&r, dest, sizeof(dest));
...
UTEST_F(io_rw, reader_file_read) {
  const char* content = "0123456789ABCDEF";
  {
    ...
    sp_io_writer_write(&w, content, 16);
  ...
  char buffer[16] = {0};
  u64 bytes = sp_io_reader_read(&r, buffer, 16);
  EXPECT_EQ(bytes, 16);
  sp_for(i, 16) {
...
UTEST_F(io_rw, reader_file_seek) {
  ...
  {
    sp_io_writer_t w = sp_io_writer_from_file(ut.file_path, SP_IO_WRITE_MODE_OVERWRITE);
    sp_io_writer_write(&w, content, 10);
    sp_io_writer_close(&w);
  }
...
UTEST_F(io_rw, writer_mem_write) {
  ...
  u64 bytes = sp_io_writer_write(&w, source, sizeof(source));
...
UTEST_F(io_rw, writer_file_write) {
  ...
  u64 bytes = sp_io_writer_write(&w, content, 9);
  EXPECT_EQ(bytes, 9);
  sp_io_writer_close(&w);
...
UTEST_F(io_rw, writer_file_overwrite) {
  ...
  {
    sp_io_writer_t w = sp_io_writer_from_file(ut.file_path, SP_IO_WRITE_MODE_OVERWRITE);
    sp_io_writer_write(&w, first, 8);
    sp_io_writer_close(&w);
  }

  {
    sp_io_writer_t w = sp_io_writer_from_file(ut.file_path, SP_IO_WRITE_MODE_OVERWRITE);
    sp_io_writer_write(&w, second, 4);
    sp_io_writer_close(&w);
  }
...
UTEST_F(io_rw, writer_file_pad) {
  sp_io_writer_t w = sp_io_writer_from_file(ut.file_path, SP_IO_WRITE_MODE_OVERWRITE);
  sp_io_writer_write(&w, "AA", 2);
  sp_io_writer_pad(&w, 3);
  sp_io_writer_write(&w, "BB", 2);
  sp_io_writer_close(&w);

  sp_io_reader_t r = sp_io_reader_from_file(ut.file_path);
  u8 result[7] = SP_ZERO_INITIALIZE();
  EXPECT_EQ(sp_io_reader_read(&r, result, 7), 7);
...
UTEST_F(io_rw, writer_dyn_write) {
  ...
  u8 data[] = {1, 2, 3, 4};
  u64 written = sp_io_writer_write(&w, data, 4);
...
UTEST_F(io_rw, writer_dyn_grows) {
  ...
  u64 written = sp_io_writer_write(&w, data, 256);
...
UTEST_F(io_rw, writer_dyn_to_str) {
  ...
  sp_io_writer_write(&w, text, 11);

  sp_str_t str = sp_io_writer_to_str(&w);
...
UTEST_F(io_rw, writer_dyn_multiple_writes) {
  ...
  sp_io_writer_write(&w, "abc", 3);
  sp_io_writer_write(&w, "def", 3);
  sp_io_writer_write(&w, "ghi", 3);
...
UTEST_F(io_rw, writer_buffered_1000_bytes) {
  ...
  sp_for(i, 1000) {
    u8 byte = (u8)(i & 0xFF);
    EXPECT_EQ(sp_io_writer_write(&w, &byte, 1), 1);
  }
  sp_io_writer_close(&w);
  ...
  sp_for(i, 1000) {
    u8 byte;
    EXPECT_EQ(sp_io_reader_read(&r, &byte, 1), 1);
...
UTEST_F(io_rw, writer_buffered_larger_than_buffer) {
  ...
  EXPECT_EQ(sp_io_writer_write(&w, data, 128), 128);
  sp_io_writer_close(&w);

  sp_io_reader_t r = sp_io_reader_from_file(ut.file_path);
  u8 result[128] = SP_ZERO_INITIALIZE();
  EXPECT_EQ(sp_io_reader_read(&r, result, 128), 128);
  sp_for(i, 128) {
...
UTEST_F(io_rw, writer_buffered_flush_empty) {
  ...
  sp_err_t err = sp_io_writer_flush(&w);
...
UTEST_F(io_rw, reader_buffered_read) {
  ...
  char result[16] = {0};
  u64 bytes = sp_io_reader_read(&r, result, 16);
  EXPECT_EQ(bytes, 16);
  sp_for(i, 16) {
...
UTEST_F(io_rw, reader_buffered_small_reads) {
  ...
  sp_for(i, 16) {
    char c;
    EXPECT_EQ(sp_io_reader_read(&r, &c, 1), 1);
