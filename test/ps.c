...
void sp_test_proc_io(sp_ps* utest_fixture, s32* utest_result, sp_test_proc_io_config_t test) {
  sp_ps_config_t config = {
    ...
    .args = {
      sp_str_lit("--fn"), sp_str_view(test_proc_function_to_cstr(test.fn)),
    },
    .io = test.io,
  };
...
sp_da(sp_env_var_t) sp_test_parse_env_output(sp_mem_t mem, u8* buffer, u64 len) {
  sp_da(sp_env_var_t) vars = sp_da_new(mem, sp_env_var_t);
...
UTEST_F(ps, concurrent_existing_fd_small_writes) {
  ...
  sp_ps_t ps_a = sp_ps_create_c(ut.mem, (sp_ps_config_cstr_t) {
    .command = get_process_path_c(ut.mem),
    .args = {
      "--fn", "pattern",
      "--stdout",
  ...
  sp_ps_t ps_b = sp_ps_create_c(ut.mem, (sp_ps_config_cstr_t) {
    .command = get_process_path_c(ut.mem),
    .args = {
      "--fn", "pattern",
      "--stdout",
  ...
  EXPECT_FALSE(analysis.interleaved);
}
...
UTEST_F(ps, concurrent_existing_fd_large_writes) {
  ...
  sp_ps_t ps_a = sp_ps_create(ut.mem, (sp_ps_config_t) {
    ...
    .args = {
      ...
      sp_str_lit("--stdout"),
      sp_str_lit("-c"), sp_str_lit("A"),
      sp_str_lit("-s"), size_str,
      sp_str_lit("-n"), count_str,
    },
    .io = {
      .in = { .mode = SP_PS_IO_MODE_NULL },
      .out = { .fd = pipes[1], .mode = SP_PS_IO_MODE_EXISTING },
...
UTEST_F(ps, create_nonexistent_binary) {
  sp_ps_t ps = sp_ps_create_c(ut.mem, (sp_ps_config_cstr_t) {
    .command = "/usr/bin/this_binary_does_not_exist_at_all"
  });
...
UTEST_F(ps, run_nonexistent_binary) {
  sp_ps_output_t result = sp_ps_run_c(ut.mem, (sp_ps_config_cstr_t) {
    .command = "/usr/bin/this_binary_does_not_exist_at_all"
  });
