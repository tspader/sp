...
void run_io_seeking_reader_file_test(int* utest_result, sp_str_t path, io_seeking_reader_file_test_t t) {
  ...
  sp_io_seeking_reader_t r = sp_zero;
  sp_io_seeking_reader_from_file_reader(&r, &fr);
...
UTEST_F(io, seeking_reader_file_seek_whence) {
  run_io_seeking_reader_file_test(utest_result, ut.file_path, (io_seeking_reader_file_test_t){
    ...
    .steps = {
      ...
      { .kind = IO_STEP_SEEK, .seek = {  3, SP_IO_SEEK_CUR, SP_OK,  7 } },
      { .kind = IO_STEP_READ, .read = {  1, SP_OK, "7" } },
      { .kind = IO_STEP_SEEK, .seek = {  0, SP_IO_SEEK_END, SP_OK, 16 } },
      { .kind = IO_STEP_SEEK, .seek = { -2, SP_IO_SEEK_END, SP_OK, 14 } },
      { .kind = IO_STEP_READ, .read = {  1, SP_OK, "E" } },
    },
  });
}
