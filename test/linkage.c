...
bool check_path(sp_str_t program) {
  return SearchPathA(SP_NULLPTR, sp_str_to_cstr(sp_mem_get_scratch(), program), SP_NULLPTR, 0, SP_NULLPTR, SP_NULLPTR) > 0;
}
#else
bool check_path(sp_str_t program) {
  return true;
...
UTEST_F_SETUP(linkage) {
  ...
  ut.source = sp_fs_join_path(ut.mem, ut.root, sp_str_lit("test/tools/linkage"));
...
sp_str_t linkage_exe(sp_str_t stem) {
  #if defined(SP_WIN32)
    return sp_fmt(sp_mem_get_scratch(), "{}.exe", sp_fmt_str(stem)).value;
  #else
    return stem;
  #endif
}
...
sp_str_t linkage_obj(sp_str_t stem) {
  #if defined(SP_WIN32)
    return sp_fmt(sp_mem_get_scratch(), "{}.obj", sp_fmt_str(stem)).value;
  #else
    return sp_fmt(sp_mem_get_scratch(), "{}.o", sp_fmt_str(stem)).value;
  #endif
}

sp_str_t linkage_shared(sp_str_t stem) {
  #if defined(SP_WIN32)
    return sp_fmt(sp_mem_get_scratch(), "{}.dll", sp_fmt_str(stem)).value;
  #else
    return sp_fmt(sp_mem_get_scratch(), "{}.so", sp_fmt_str(stem)).value;
  #endif
}

sp_str_t linkage_static(sp_str_t stem) {
  #if defined(SP_WIN32)
    return sp_fmt(sp_mem_get_scratch(), "{}.lib", sp_fmt_str(stem)).value;
  #else
    return sp_fmt(sp_mem_get_scratch(), "{}.a", sp_fmt_str(stem)).value;
  #endif
}

void linkage_add_win32_link_libs(sp_ps_config_t* cfg) {
  #if defined(SP_WIN32)
    sp_ps_config_add_arg(sp_mem_get_scratch(), cfg, sp_str_lit("/link"));
    sp_ps_config_add_arg(sp_mem_get_scratch(), cfg, sp_str_lit("shell32.lib"));
  #else
    SP_UNUSED(cfg);
  #endif
}
...
bool compile_to_exe(linkage* ctx, const c8* file, sp_str_t output) {
    ...
    sp_ps_config_t cfg = {
      ...
      .args = {
        sp_str_lit("/nologo"),
        sp_str_lit("/TC"),
        sp_fmt(sp_mem_get_scratch(), "/I{}", sp_fmt_str(ctx->root)).value,
        sp_fs_join_path(sp_mem_get_scratch(), ctx->source, sp_str_view(file)),
        sp_fmt(sp_mem_get_scratch(), "/Fe:{}", sp_fmt_str(output)).value,
      },
    };

    linkage_add_win32_link_libs(&cfg);
    sp_ps_output_t out = sp_ps_run(sp_mem_get_scratch(), cfg);

    return !out.status.exit_code;
  #else
  sp_ps_output_t out = sp_ps_run(sp_mem_get_scratch(), (sp_ps_config_t) {
...
bool compile_to_object(linkage* ctx, const c8* file, sp_str_t output) {
    ...
    sp_ps_output_t out = sp_ps_run(sp_mem_get_scratch(), (sp_ps_config_t) {
      ...
      .args = {
        ...
        sp_fmt(sp_mem_get_scratch(), "/I{}", sp_fmt_str(ctx->root)).value,
        sp_fs_join_path(sp_mem_get_scratch(), ctx->source, sp_str_view(file)),
        sp_fmt(sp_mem_get_scratch(), "/Fo:{}", sp_fmt_str(output)).value,
      },
    });
...
bool compile_objects_to_exe(sp_str_t output, sp_str_t* objs, u32 len) {
    ...
    sp_ps_config_t cfg = {
      .command = sp_str_lit("cl"),
      .args = { sp_str_lit("/nologo"), sp_fmt(sp_mem_get_scratch(), "/Fe:{}", sp_fmt_str(output)).value },
    };

    sp_for(it, len) {
      sp_ps_config_add_arg(sp_mem_get_scratch(), &cfg, objs[it]);
    }
...
bool compile_to_linked_exe(linkage* ctx, const c8* file, sp_str_t output, sp_str_t* libs, u32 len) {
    ...
    sp_for(it, len) {
      sp_ps_config_add_arg(sp_mem_get_scratch(), &cfg, libs[it]);
    }
...
bool create_archive(linkage* ctx, sp_str_t archive, sp_str_t* objs, u32 len) {
    ...
    sp_ps_config_t cfg = {
      ...
      .args = {
        sp_str_lit("/nologo"),
        sp_fmt(sp_mem_get_scratch(), "/OUT:{}", sp_fmt_str(archive)).value,
      },
    };
  ...
  sp_ps_config_add_arg(sp_mem_get_scratch(), &cfg, archive);
  for (u32 it = 0; it < len; it++) {
    sp_ps_config_add_arg(sp_mem_get_scratch(), &cfg, objs[it]);
  }
...
bool is_symbol_in_binary(sp_str_t binary, sp_str_t symbol) {
  #if defined(SP_WIN32)
    HMODULE module = LoadLibraryA(sp_str_to_cstr(sp_mem_get_scratch(), binary));
    if (!module) {
      return false;
    }

    bool found = GetProcAddress(module, sp_str_to_cstr(sp_mem_get_scratch(), symbol)) != SP_NULLPTR;
    FreeLibrary(module);
    return found;
  #else
    sp_ps_output_t nm = sp_ps_run(sp_mem_get_scratch(), (sp_ps_config_t){
...
UTEST_F(linkage, shared_lib) {
  ...
  #if defined(SP_WIN32)
    sp_ps_output_t out = sp_ps_run(ut.mem, (sp_ps_config_t){
      .command = sp_str_lit("cl"),
      .args = {
        sp_str_lit("/nologo"),
        sp_str_lit("/LD"),
        ...
        sp_fs_join_path(ut.mem, ut.source, sp_str_lit("lib-impl.c")),
        sp_fmt(ut.mem, "/I{}", sp_fmt_str(ut.root)).value,
        sp_fmt(ut.mem, "/Fe:{}", sp_fmt_str(so)).value,
        sp_str_lit("/link"),
  ...
  sp_str_t obj = linkage_obj(sp_test_file_path(&ut.files, sp_str_lit("cpp-main")));
    ...
    sp_ps_output_t out = sp_ps_run(ut.mem, (sp_ps_config_t){
      ...
      .args = {
        ...
        sp_str_lit("/std:c++20"),
        sp_str_lit("/c"),
        sp_fs_join_path(ut.mem, ut.source, sp_str_lit("cpp-compat.c")),
        sp_fmt(ut.mem, "/I{}", sp_fmt_str(ut.root)).value,
        sp_fmt(ut.mem, "/Fo:{}", sp_fmt_str(obj)).value,
        sp_str_lit("/WX"),
      },
    });
  #else
