...
UTEST(cstr, all_variations) {
  ...
  c8* copy = sp_cstr_copy(sp_mem_get_scratch(), original);
  ASSERT_TRUE(sp_cstr_equal(copy, original));
  ASSERT_NE(copy, original);

  c8* partial = sp_cstr_copy_n(sp_mem_get_scratch(), original, 5);
  ...
  c8* null_copy = sp_cstr_copy(sp_mem_get_scratch(), SP_NULLPTR);
...
UTEST(str, conversion_functions) {
  ...
  c8* cstr = sp_str_to_cstr(sp_mem_get_scratch(), str);
...
UTEST(str, string_copy_operations) {
  ...
  sp_str_t copy = sp_str_copy(sp_mem_get_scratch(), original);
  ASSERT_EQ(copy.len, original.len);
  ASSERT_TRUE(sp_str_equal(copy, original));
  ASSERT_NE(copy.data, original.data);

  sp_str_t from_cstr = sp_str_from_cstr(sp_mem_get_scratch(), "Test String");
  ASSERT_EQ(from_cstr.len, 11);
  SP_EXPECT_STR_EQ_CSTR(from_cstr, "Test String");

  sp_str_t partial = sp_str_from_cstr_n(sp_mem_get_scratch(), "Hello World", 5);
...
UTEST(str, string_creation) {
  ...
  sp_str_t allocated = sp_str_alloc(sp_mem_get_scratch(), 100);
...
UTEST(str, to_upper_and_replace) {
  {
    ...
    sp_str_t uppercase = sp_str_to_upper(sp_mem_get_scratch(), lowercase);
    SP_EXPECT_STR_EQ_CSTR(uppercase, "HELLO WORLD!");
  };

  {
    sp_str_t lower = sp_str_lit("caf\xC3\xA9 123 {[]}");
    SP_EXPECT_STR_EQ_CSTR(sp_str_to_upper(sp_mem_get_scratch(), lower), "CAF\xC3\xA9 123 {[]}");
  }

  {
    sp_str_t upper = sp_str_lit("CAF\xC3\xA9 123 {[]}");
    SP_EXPECT_STR_EQ_CSTR(sp_str_to_lower(sp_mem_get_scratch(), upper), "caf\xC3\xA9 123 {[]}");
  }

  sp_str_t mixed = sp_str_lit("HeLLo WoRLd!");
  sp_str_t upper_mixed = sp_str_to_upper(sp_mem_get_scratch(), mixed);
  SP_EXPECT_STR_EQ_CSTR(upper_mixed, "HELLO WORLD!");

  sp_str_t original = sp_str_lit("hello world");
  sp_str_t replaced = sp_str_replace_c8(sp_mem_get_scratch(), original, 'l', 'X');
  SP_EXPECT_STR_EQ_CSTR(replaced, "heXXo worXd");

  sp_str_t no_match = sp_str_replace_c8(sp_mem_get_scratch(), original, 'z', 'X');
  SP_EXPECT_STR_EQ_CSTR(no_match, "hello world");

  // all same char
  SP_EXPECT_STR_EQ_CSTR(sp_str_replace_c8(sp_mem_get_scratch(), sp_str_lit("aaa"), 'a', 'b'), "bbb");

  // empty string
  SP_EXPECT_STR_EQ_CSTR(sp_str_replace_c8(sp_mem_get_scratch(), sp_str_lit(""), 'a', 'b'), "");

  // same from/to (no-op)
  SP_EXPECT_STR_EQ_CSTR(sp_str_replace_c8(sp_mem_get_scratch(), sp_str_lit("abc"), 'a', 'a'), "abc");

  // single char string
  SP_EXPECT_STR_EQ_CSTR(sp_str_replace_c8(sp_mem_get_scratch(), sp_str_lit("x"), 'x', 'y'), "y");

  // first and last positions
  SP_EXPECT_STR_EQ_CSTR(sp_str_replace_c8(sp_mem_get_scratch(), sp_str_lit("/path/to/file/"), '/', '\\'), "\\path\\to\\file\\");
}
...
UTEST(str, concat) {
  SP_EXPECT_STR_EQ_CSTR(sp_str_concat(sp_mem_get_scratch(), sp_str_lit("Jerry"), sp_str_lit("Garcia")), "JerryGarcia");
  SP_EXPECT_STR_EQ_CSTR(sp_str_concat(sp_mem_get_scratch(), sp_str_lit("Jerry"), sp_str_lit("")), "Jerry");
  SP_EXPECT_STR_EQ_CSTR(sp_str_concat(sp_mem_get_scratch(), sp_str_lit(""), sp_str_lit("Jerry")), "Jerry");
}
...
UTEST(str, split_c8) {
  {
    sp_da(sp_str_t) parts = sp_str_split_c8(sp_mem_get_scratch(), sp_str_lit("hello,world,test"), ',');
  ...
  {
    sp_da(sp_str_t) parts = sp_str_split_c8(sp_mem_get_scratch(), sp_str_lit("/home/user/file.txt"), '/');
  ...
  {
    sp_da(sp_str_t) parts = sp_str_split_c8(sp_mem_get_scratch(), sp_str_lit("a,,b"), ',');
    ASSERT_EQ(sp_da_size(parts), 3);
  ...
  {
    sp_da(sp_str_t) parts = sp_str_split_c8(sp_mem_get_scratch(), sp_str_lit("hello"), ',');
    ASSERT_EQ(sp_da_size(parts), 1);
    SP_EXPECT_STR_EQ_CSTR(parts[0], "hello");
  }

  {
    sp_da(sp_str_t) parts = sp_str_split_c8(sp_mem_get_scratch(), sp_str_lit(""), ',');
    EXPECT_EQ(parts, SP_NULLPTR);
  }

  {
    sp_da(sp_str_t) parts = sp_str_split_c8(sp_mem_get_scratch(), sp_str_lit(",hello,world,"), ',');
    ASSERT_EQ(sp_da_size(parts), 4);
    SP_EXPECT_STR_EQ_CSTR(parts[0], "");
  ...
  {
    sp_da(sp_str_t) parts = sp_str_split_c8(sp_mem_get_scratch(), sp_str_lit("x"), ',');
    ASSERT_EQ(sp_da_size(parts), 1);
...
UTEST(str, pad) {
  SP_EXPECT_STR_EQ(sp_str_pad(sp_mem_get_scratch(), sp_str_lit("hello"), 10), sp_str_lit("hello     "));
  SP_EXPECT_STR_EQ(sp_str_pad(sp_mem_get_scratch(), sp_str_lit("hi"), 5), sp_str_lit("hi   "));

  SP_EXPECT_STR_EQ(sp_str_pad(sp_mem_get_scratch(), sp_str_lit("hello world"), 5), sp_str_lit("hello world"));

  SP_EXPECT_STR_EQ(sp_str_pad(sp_mem_get_scratch(), sp_str_lit("hello"), 5), sp_str_lit("hello"));

  SP_EXPECT_STR_EQ(sp_str_pad(sp_mem_get_scratch(), sp_str_lit(""), 5), sp_str_lit("     "));

  SP_EXPECT_STR_EQ(sp_str_pad(sp_mem_get_scratch(), sp_str_lit("hello"), 0), sp_str_lit("hello"));
}
...
UTEST(str, pad_to_longest) {
  {
    ...
    sp_da(sp_str_t) padded = sp_str_pad_to_longest(sp_mem_get_scratch(), strings, 3);
...

  {
    ...
    sp_da(sp_str_t) padded = sp_str_pad_to_longest(sp_mem_get_scratch(), strings, 1);
...
UTEST(sp_str_from_cstr, string_from_cstr) {
  {
    const c8* cstr = "hello world";
    sp_str_t str = sp_str_from_cstr(sp_mem_get_scratch(), cstr);
  ...
  {
    sp_str_t str = sp_str_from_cstr(sp_mem_get_scratch(), "");
    ASSERT_EQ(str.len, 0);
    SP_EXPECT_STR_EQ(str, sp_str_lit(""));
  }

  {
    sp_str_t str = sp_str_from_cstr(sp_mem_get_scratch(), SP_NULLPTR);
    ASSERT_EQ(str.len, 0);
  ...
  {
    c8 buffer[] = "mutable";
    sp_str_t str = sp_str_from_cstr(sp_mem_get_scratch(), buffer);
    buffer[0] = 'M';
    ASSERT_EQ(str.data[0], 'm');
  }

  {
    sp_str_t str = sp_str_from_cstr_n(sp_mem_get_scratch(), "hello world", 5);
    ASSERT_EQ(str.len, 5);
