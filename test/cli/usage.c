...
static void cli_usage_write_file(sp_str_t path, sp_str_t content) {
...
static void run_cli_usage_test(s32* utest_result, sp_mem_t mem, cli_usage_test_t t) {
  ...
  sp_cli_t cli = { .cmd = &t.cmd, .io = &io.base, .theme = { .mode = SP_CLI_THEME_REPLACE } };
  ...
  if (!sp_str_empty(sp_os_env_get(sp_str_lit("SP_CLI_TEST_UPDATE")))) {
    ...
    cli_usage_write_file(path, actual);
  ...
  if (!sp_str_equal(actual, expected)) {
    ...
    cli_usage_write_file(actual_path, actual);
