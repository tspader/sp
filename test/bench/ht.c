...
static void run_benchmarks(bench_t* benches, u32 num_benches) {
  ...
  sp_da_for(results, i) {
    ...
    sp_str_t sp_time_str = sp_str_pad_a(sp_mem_get_scratch(), sp_fmt_a(sp_context_get_allocator(), "{}ms", sp_fmt_float(sp_ms)).value, time_width);
    sp_str_t stb_time_str = sp_str_pad_a(sp_mem_get_scratch(), sp_fmt_a(sp_context_get_allocator(), "{}ms", sp_fmt_float(stb_ms)).value, time_width);
    sp_str_t ratio_color = color_for_ratio(ratio);
    sp_str_t ratio_str = sp_str_pad_a(sp_mem_get_scratch(), sp_fmt_a(sp_context_get_allocator(), "{}x", sp_fmt_float(ratio)).value, ratio_width);
