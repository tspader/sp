...
int main(int argc, char** argv) {
  ...
  for (u32 i = 0; i < num_cases; i++) {
    ...
    sp_str_t name = sp_fmt_a(sp_context_get_allocator(), "{}_glob", sp_fmt_cstr(case_names[i])).value;
  ...
  for (u32 i = 0; i < num_cases; i++) {
    ...
    sp_str_t name = sp_fmt_a(sp_context_get_allocator(), "{}_globset", sp_fmt_cstr(case_names[i])).value;
    sp_da_push(results, ((bench_result_t){.name = name, .ns_per_op = ns}));
  }
