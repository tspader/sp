...
static void run_sys_pipe_test(s32* utest_result, sys_pipe_test_t t) {
  ...
  ASSERT_EQ(sp_sys_pipe(&p, t.desc), SP_OK);
  sp_sys_fd_t r = p.r;
  sp_sys_fd_t w = p.w;
...
// it is a contract violation, so the WOULD_BLOCK tests are POSIX-only.
#if !defined(SP_WIN32)
...
UTEST_F(sys_pipe, write_to_full_pipe_reports_would_block) {
  sp_sys_pipe_t p = sp_zero;
  ASSERT_EQ(sp_sys_pipe(&p, (sp_sys_pipe_desc_t) { .w = { SP_SYS_NONBLOCKING } }), SP_OK);

  u8 chunk [4096] = sp_zero;
  sp_err_t err = SP_OK;
  while (err == SP_OK) {
    u64 n = 0;
    err = sp_sys_write(p.w, chunk, sizeof(chunk), &n);
