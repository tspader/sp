...
typedef struct {
  ...
  union {
    ...
    struct { u32 slot; const c8* path; } open_dir;
    ...
    struct { const c8* path; sp_err_t err; } mkdir;
    struct { const c8* path; sp_err_t err; } rmdir;
    struct { const c8* path; sp_err_t err; } unlink;
    struct { const c8* existing; sys_dir_t existing_dir; const c8* alias; sys_dir_t alias_dir; sp_err_t err; } link;
  };
} sys_step_t;
...
static void sys_expect_err(s32* utest_result, const c8* label, sp_err_t err, sp_err_t expect) {
  if (err != expect) {
...
static void sys_expect_meta(s32* utest_result, const c8* label, sp_err_t err, sp_err_t expect, const sp_sys_file_meta_t* meta, sp_fs_kind_t kind, s64 size) {
  sys_expect_err(utest_result, label, err, expect);
  if (err || expect) return;
  ...
  sp_carr_for(t.steps, it) {
    ...
    switch (step->kind) {
      ...
      case SYS_STEP_OPEN: {
        ...
        sys_expect_err(utest_result, "open", err, step->open.err);
        if (err == SP_OK) {
          if (step->open.err) {
            sp_sys_close(fd);
          }
      ...
      case SYS_STEP_PREAD: {
        ...
        sys_expect_err(utest_result, "pread", err, SP_OK);
        if (err == SP_OK) {
      ...
      case SYS_STEP_WRITE: {
        ...
        if (err == SP_OK && !step->write.err && n != len) {
      ...
      case SYS_STEP_PWRITE: {
        ...
        sys_expect_err(utest_result, "pwrite", err, SP_OK);
        if (err == SP_OK && n != len) {
      ...
      case SYS_STEP_RENAME: {
        ...
        sys_expect_err(utest_result, "rename", err, step->rename.err);
        break;
      }
      case SYS_STEP_LSTAT: {
        sp_sys_file_meta_t meta = sp_zero;
        sp_err_t err = sp_sys_get_link_metadata_s(sandbox_fd, sp_cstr_as_str(step->lstat.path), &meta);
        sys_expect_meta(utest_result, "lstat", err, step->lstat.err, &meta, step->lstat.kind, 0);
        break;
      }
      case SYS_STEP_STAT: {
        sp_sys_file_meta_t meta = sp_zero;
        sp_err_t err = sp_sys_get_path_metadata_s(sandbox_fd, sp_cstr_as_str(step->stat.path), &meta);
        sys_expect_meta(utest_result, "stat", err, step->stat.err, &meta, step->stat.kind, step->stat.size);
        break;
      }
      case SYS_STEP_FSTAT: {
        sp_sys_file_meta_t meta = sp_zero;
        sp_err_t err = sp_sys_get_file_metadata(fds[step->fstat.slot], &meta);
        sys_expect_meta(utest_result, "fstat", err, SP_OK, &meta, step->fstat.kind, step->fstat.size);
        break;
      }
      case SYS_STEP_MKDIR: {
      ...
      case SYS_STEP_RMDIR: {
        sp_err_t err = sp_sys_rmdir_s(sandbox_fd, sp_cstr_as_str(step->rmdir.path));
        sys_expect_err(utest_result, "rmdir", err, step->rmdir.err);
        break;
      }
      case SYS_STEP_UNLINK: {
        sp_err_t err = sp_sys_unlink_s(sandbox_fd, sp_cstr_as_str(step->unlink.path));
        sys_expect_err(utest_result, "unlink", err, step->unlink.err);
        break;
      }
      case SYS_STEP_LINK: {
        sp_err_t err = sp_sys_link_s(
        ...
        sys_expect_err(utest_result, "link", err, step->link.err);
        break;
      }
    }
  }
