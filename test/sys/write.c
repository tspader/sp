...
UTEST_F(sys_write, append_writes_at_current_eof) {
  ...
  run_sys_test(utest_result, (sys_test_t) {
    ...
    .steps = {
      { .kind = SYS_STEP_OPEN, .open = { .path = "file.bin", .flags = SP_O_WRONLY | SP_O_APPEND | SP_O_BINARY } },
      { .kind = SYS_STEP_OPEN, .open = { .slot = 1, .path = "file.bin", .flags = SP_O_WRONLY | SP_O_BINARY } },
