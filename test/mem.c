...
void run_mem_copy_test(int* utest_result, mem_copy_test_t* t) {
  u8 dest[512] = {0};
  u8 sentinel = 0xAB;

  // fill dest with sentinel
  for (u32 i = 0; i < sizeof(dest); i++) dest[i] = sentinel;

  sp_mem_copy(t->src, dest, t->copy_len);

  // verify copied region matches source
  for (u32 i = 0; i < t->copy_len; i++) {
    EXPECT_EQ(dest[i], t->src[i]);
  }

  // verify byte after copied region is untouched
...
void run_mem_move_test(int* utest_result, mem_move_test_t* t) {
  u8 buf[512];
  u8 expected[512];
  ...
  // memmove semantics: copy src to dst even if they overlap
  u8 tmp[512];
...
void run_mem_is_equal_test(int* utest_result, mem_is_equal_test_t* t) {
...
void run_mem_fill_test(int* utest_result, mem_fill_test_t* t) {
  u8 buffer[512];
  u8 sentinel = 0xCD;
  ...
  // verify filled region has repeating pattern
  ...
  // verify bytes after filled region are untouched (partial fill doesn't happen)
  for (u32 i = filled; i < t->buffer_size; i++) {
    EXPECT_EQ(buffer[i], sentinel);
  }

  // verify beyond buffer_size is untouched
...
void run_mem_fill_u8_test(int* utest_result, mem_fill_u8_test_t* t) {
  u8 buffer[512];
  u8 sentinel = 0xCD;
  for (u32 i = 0; i < sizeof(buffer); i++) buffer[i] = sentinel;
...
void run_mem_zero_test(int* utest_result, mem_zero_test_t* t) {
  u8 buffer[512];
  u8 sentinel = 0xEF;
  for (u32 i = 0; i < sizeof(buffer); i++) buffer[i] = sentinel;
...
struct mem { u8 placeholder; };
UTEST_F_SETUP(mem) { (void)utest_fixture; }
UTEST_F_TEARDOWN(mem) { (void)utest_fixture; }
...
UTEST_F(mem, move_overlap_forward) {
  // src at 0, dst at 4, len 16: dst overlaps src (forward copy needed)
  run_mem_move_test(utest_result, &(mem_move_test_t){
...
UTEST_F(mem, move_overlap_backward) {
  // src at 4, dst at 0, len 16: dst overlaps src (backward copy needed)
  run_mem_move_test(utest_result, &(mem_move_test_t){
...
UTEST_F(mem, fill_pattern_larger_than_buffer) {
  // fill_size=4, buffer_size=3 → nothing fits, buffer untouched
  run_mem_fill_test(utest_result, &(mem_fill_test_t){
...
UTEST_F(mem, fill_exact_multiple) {
  // 8 bytes with 4-byte pattern = exactly 2 copies
  run_mem_fill_test(utest_result, &(mem_fill_test_t){
...
UTEST_F(mem, fill_remainder_truncated) {
  // 10 bytes with 4-byte pattern = 2 full copies (8 bytes), last 2 untouched
  run_mem_fill_test(utest_result, &(mem_fill_test_t){
