...
static sp_str_ht(_sp_fmt_directive_t) _sp_fmt_directives = SP_NULLPTR;

typedef struct {
  const c8* str;
  _sp_fmt_spec_t expected;
  sp_err_t err;
} test_t;

void run_case(s32* utest_result, test_t test) {
  _sp_fmt_spec_t specifier = SP_ZERO_INITIALIZE();
  EXPECT_EQ(sp_fmt_parse_specifier(sp_str_view(test.str), &specifier), test.err);
...
UTEST(sp_fmt_parse, err_dynamic_precision_without_dot) {
  _sp_fmt_spec_t specifier = SP_ZERO_INITIALIZE();
  EXPECT_EQ(sp_fmt_parse_specifier(sp_str_view("{:5$}"), &specifier), SP_ERR_FMT_BAD_PLACEHOLDER);
}
...
UTEST(sp_fmt_parse, err_dot_without_digits) {
  _sp_fmt_spec_t specifier = SP_ZERO_INITIALIZE();
  EXPECT_EQ(sp_fmt_parse_specifier(sp_str_view("{:5.}"), &specifier), SP_ERR_FMT_BAD_PRECISION);
}

UTEST(sp_fmt_parse, err_trailing_junk) {
  run_case(utest_result, (test_t) {
    .str = "{:5}x",
    .err = SP_ERR_LAZY,
    .expected = SP_ZERO_INITIALIZE(),
  });
}

UTEST(sp_fmt_parse_directive, single) {
  _sp_fmt_spec_t spec = SP_ZERO_INITIALIZE();
  sp_err_t err = sp_fmt_parse_specifier(sp_str_view("{:red}"), &spec);
  EXPECT_EQ(err, SP_OK);
...
UTEST(sp_fmt_parse_directive, after_width) {
  _sp_fmt_spec_t spec = SP_ZERO_INITIALIZE();
  sp_err_t err = sp_fmt_parse_specifier(sp_str_view("{:10 :red}"), &spec);
  EXPECT_EQ(err, SP_OK);
...
UTEST(sp_fmt_parse_directive, full_spec) {
  _sp_fmt_spec_t spec = SP_ZERO_INITIALIZE();
  sp_err_t err = sp_fmt_parse_specifier(sp_str_view("{:*^9.2 :red}"), &spec);
  EXPECT_EQ(err, SP_OK);
...
UTEST(sp_fmt_parse_directive, multiple) {
  _sp_fmt_spec_t spec = SP_ZERO_INITIALIZE();
  sp_err_t err = sp_fmt_parse_specifier(sp_str_view("{:red :bold :upper}"), &spec);
  EXPECT_EQ(err, SP_OK);
...
UTEST(sp_fmt_parse_directive, max_count) {
  _sp_fmt_spec_t spec = SP_ZERO_INITIALIZE();
  sp_err_t err = sp_fmt_parse_specifier(sp_str_view("{:a :b :c :d :e :f :g :h}"), &spec);
  EXPECT_EQ(err, SP_OK);
  EXPECT_EQ(spec.directive_count, _SP_FMT_MAX_DIRECTIVES);
}

UTEST(sp_fmt_parse_directive, err_too_many) {
  _sp_fmt_spec_t spec = SP_ZERO_INITIALIZE();
  sp_err_t err = sp_fmt_parse_specifier(sp_str_view("{:a :b :c :d :e :f :g :h :i}"), &spec);
  EXPECT_EQ(err, SP_ERR_FMT_TOO_MANY_DIRECTIVES);
}

UTEST(sp_fmt_parse_directive, err_empty_name) {
  _sp_fmt_spec_t spec = SP_ZERO_INITIALIZE();
  sp_err_t err = sp_fmt_parse_specifier(sp_str_view("{:10 :}"), &spec);
  EXPECT_EQ(err, SP_ERR_FMT_BAD_DIRECTIVE);
}

UTEST(sp_fmt_parse_directive, err_bad_char) {
  _sp_fmt_spec_t spec = SP_ZERO_INITIALIZE();
  sp_err_t err = sp_fmt_parse_specifier(sp_str_view("{:10 :red!}"), &spec);
  EXPECT_EQ(err, SP_ERR_FMT_BAD_PLACEHOLDER);
}

UTEST(sp_fmt_parse_directive, err_no_colon) {
  _sp_fmt_spec_t spec = SP_ZERO_INITIALIZE();
  sp_err_t err = sp_fmt_parse_specifier(sp_str_view("{:10 red}"), &spec);
  EXPECT_EQ(err, SP_ERR_FMT_BAD_PLACEHOLDER);
}
...
UTEST(sp_fmt_parse_directive, digit_in_tail) {
  _sp_fmt_spec_t spec = SP_ZERO_INITIALIZE();
  sp_err_t err = sp_fmt_parse_specifier(sp_str_view("{:base64}"), &spec);
  EXPECT_EQ(err, SP_OK);
  EXPECT_EQ(spec.directive_count, 1);
...
UTEST(sp_fmt_parse_directive, hyphen_in_name) {
  _sp_fmt_spec_t spec = SP_ZERO_INITIALIZE();
  sp_err_t err = sp_fmt_parse_specifier(sp_str_view("{:utf-8}"), &spec);
  EXPECT_EQ(err, SP_OK);
  EXPECT_EQ(spec.directive_count, 1);
...
UTEST(sp_fmt_parse_directive, underscore_in_name) {
  _sp_fmt_spec_t spec = SP_ZERO_INITIALIZE();
  sp_err_t err = sp_fmt_parse_specifier(sp_str_view("{:http_url}"), &spec);
  EXPECT_EQ(err, SP_OK);
  EXPECT_EQ(spec.directive_count, 1);
...
UTEST(sp_fmt_parse_directive, err_leading_digit) {
  _sp_fmt_spec_t spec = SP_ZERO_INITIALIZE();
  sp_err_t err = sp_fmt_parse_specifier(sp_str_view("{: :4red}"), &spec);
  EXPECT_EQ(err, SP_ERR_FMT_BAD_DIRECTIVE);
}
...
UTEST(sp_fmt_parse_directive, err_no_space_between_directives) {
  _sp_fmt_spec_t spec = SP_ZERO_INITIALIZE();
  sp_err_t err = sp_fmt_parse_specifier(sp_str_view("{:red:bold}"), &spec);
  EXPECT_EQ(err, SP_ERR_FMT_BAD_PLACEHOLDER);
}
...
UTEST(sp_fmt_render, f64_custom_precision_via_spec) {
  _sp_fmt_arg_t arg = _sp_fmt_f64(3.14159);
...
UTEST(sp_fmt_render, ptr_null) {
  sp_str_t got = render_value_to_str(_sp_fmt_ptr(SP_NULLPTR));
  EXPECT_TRUE(sp_str_equal_cstr(got, "0x0"));
}

UTEST(sp_fmt_render, ptr_nonzero) {
  sp_str_t got = render_value_to_str(_sp_fmt_ptr((void*)(uintptr_t)0xdeadbeefULL));
  EXPECT_TRUE(sp_str_equal_cstr(got, "0xdeadbeef"));
}

static sp_str_t apply_spec_to_str(const c8* content, _sp_fmt_spec_t spec) {
  sp_str_builder_t b = SP_ZERO_INITIALIZE();
  _sp_fmt_apply_spec(&b, sp_str_view(content), spec);
  return sp_str_builder_to_str(&b);
}

UTEST(sp_fmt_pad, no_width) {
  sp_str_t got = apply_spec_to_str("hello", SP_ZERO_STRUCT(_sp_fmt_spec_t));
  EXPECT_TRUE(sp_str_equal_cstr(got, "hello"));
}

UTEST(sp_fmt_pad, content_longer_than_width) {
  sp_str_t got = apply_spec_to_str("hello", (_sp_fmt_spec_t){ .width = 3 });
  EXPECT_TRUE(sp_str_equal_cstr(got, "hello"));
}

UTEST(sp_fmt_pad, right_align_default) {
  sp_str_t got = apply_spec_to_str("42", (_sp_fmt_spec_t){ .width = 6 });
  EXPECT_TRUE(sp_str_equal_cstr(got, "    42"));
}

UTEST(sp_fmt_pad, left_align_fill) {
  sp_str_t got = apply_spec_to_str("42", (_sp_fmt_spec_t){
...
UTEST(sp_fmt_pad, center_even) {
  sp_str_t got = apply_spec_to_str("hi", (_sp_fmt_spec_t){
...
UTEST(sp_fmt_pad, wrapped_padding_outside) {
  sp_str_builder_t b = SP_ZERO_INITIALIZE();
  _sp_fmt_apply_spec_wrapped(&b,
    sp_str_view("<"),
    sp_str_view("42"),
    sp_str_view(">"),
    (_sp_fmt_spec_t){ .width = 6 }
  );
  sp_str_t got = sp_str_builder_to_str(&b);
  EXPECT_TRUE(sp_str_equal_cstr(got, "    <42>"));
}
...
UTEST(sp_fmt_pad, wrapped_center) {
  ...
  _sp_fmt_apply_spec_wrapped(&b,
    ...
    sp_str_view("hi"),
    sp_str_view("]"),
    (_sp_fmt_spec_t){ .width = 8, .align = SP_FMT_ALIGN_CENTER, .fill = '*' }
  );
  sp_str_t got = sp_str_builder_to_str(&b);
  EXPECT_TRUE(sp_str_equal_cstr(got, "***[hi]***"));
}

static void _test_before_lt(sp_str_builder_t* b, _sp_fmt_arg_t* arg) {
  (void)arg;
  sp_str_builder_append_cstr(b, "<");
}

static void _test_after_gt(sp_str_builder_t* b, sp_mem_slice_t content, _sp_fmt_arg_t* arg) {
  (void)content; (void)arg;
  sp_str_builder_append_cstr(b, ">");
}

static void _test_render_x(sp_str_builder_t* b, _sp_fmt_arg_t* arg) {
  (void)arg;
  sp_str_builder_append_cstr(b, "X");
}

static void _test_after_upper(sp_str_builder_t* b, sp_mem_slice_t content, _sp_fmt_arg_t* arg) {
  (void)b; (void)arg;
  sp_mem_slice_for(content, k) {
    u8 c = content.data[k];
    if (c >= 'a' && c <= 'z') content.data[k] = c - 32;
  }
}
...
static void _test_before_a(sp_str_builder_t* b, _sp_fmt_arg_t* arg) {
  (void)arg;
  sp_str_builder_append_cstr(b, "[a");
  sp_str_builder_append_cstr(&_test_log, "ba");
}

static void _test_after_a(sp_str_builder_t* b, sp_mem_slice_t content, _sp_fmt_arg_t* arg) {
  (void)content; (void)arg;
  sp_str_builder_append_cstr(b, "a]");
  sp_str_builder_append_cstr(&_test_log, "aa");
}

static void _test_before_b(sp_str_builder_t* b, _sp_fmt_arg_t* arg) {
  (void)arg;
  sp_str_builder_append_cstr(b, "[b");
  sp_str_builder_append_cstr(&_test_log, "bb");
}

static void _test_after_b(sp_str_builder_t* b, sp_mem_slice_t content, _sp_fmt_arg_t* arg) {
  (void)content; (void)arg;
  sp_str_builder_append_cstr(b, "b]");
  sp_str_builder_append_cstr(&_test_log, "ab");
}

static void _test_render_y(sp_str_builder_t* b, _sp_fmt_arg_t* arg) {
  (void)arg;
...
UTEST(sp_fmt_directive, register_and_lookup) {
  ...
  EXPECT_TRUE(got->before == _test_before_lt);
  EXPECT_TRUE(got->after == _test_after_gt);

  sp_str_t rendered = sp_fmt("{:wrap}", _sp_fmt_cstr("ok"));
...
UTEST(sp_fmt_directive, lookup_unknown_returns_null) {
  sp_fmt_directive_reset();
  _sp_fmt_directive_t* got = sp_fmt_directive_lookup(sp_str_view("nope"));
...
UTEST(sp_fmt_directive, err_unknown_directive) {
  ...
  EXPECT_EQ(sp_fmt_e(&str, "{:missing}", _sp_fmt_int(42)), SP_ERR_FMT_UNKNOWN_DIRECTIVE);
}
...
UTEST(sp_fmt_directive, ordering_bracket_nested) {
  ...
  sp_str_t got = sp_fmt("{:a :b}", _sp_fmt_cstr("x"));
...
static void _test_render_prefixed(sp_str_builder_t* b, _sp_fmt_arg_t* arg) {
  (void)arg;
...
UTEST(sp_fmt_parse_directive, err_alpha_after_digit) {
  _sp_fmt_spec_t spec = SP_ZERO_INITIALIZE();
  sp_err_t err = sp_fmt_parse_specifier(sp_str_view("{:5red}"), &spec);
  EXPECT_EQ(err, SP_ERR_FMT_BAD_PLACEHOLDER);
}
...
UTEST(sp_fmt_v, escaped_braces_around_placeholder) {
  sp_str_t got = sp_fmt("{{{:5}}}", _sp_fmt_int(42));
  EXPECT_TRUE(sp_str_equal_cstr(got, "{   42}"));
}

UTEST(sp_fmt_v, dynamic_width) {
  sp_str_t got = sp_fmt("{:$}", _sp_fmt_int(6), _sp_fmt_int(42));
  EXPECT_TRUE(sp_str_equal_cstr(got, "    42"));
}

UTEST(sp_fmt_v, dynamic_fill_center) {
  sp_str_t got = sp_fmt("{:$^9}", _sp_fmt_int('*'), _sp_fmt_int(42));
  EXPECT_TRUE(sp_str_equal_cstr(got, "***42****"));
}
...
UTEST(sp_fmt_v, dynamic_precision) {
  sp_str_t got = sp_fmt("{:.$}", _sp_fmt_int(3), _sp_fmt_f64(3.14159));
  EXPECT_TRUE(sp_str_equal_cstr(got, "3.142"));
}

UTEST(sp_fmt_v, dynamic_width_with_literal_precision) {
  sp_str_t got = sp_fmt("{:$.2}", _sp_fmt_int(8), _sp_fmt_f64(1.5));
  EXPECT_TRUE(sp_str_equal_cstr(got, "    1.50"));
}

UTEST(sp_fmt_v, err_parse_stops_formatting) {
  sp_str_t str = sp_zero();
  EXPECT_EQ(sp_fmt_e(&str, "a {:5.} b {}", _sp_fmt_int(99)), SP_ERR_FMT_BAD_PRECISION);
}

UTEST(sp_fmt_v, err_unterminated_placeholder) {
  sp_str_t got = sp_fmt("hi {nope", _sp_fmt_int(1));
  EXPECT_TRUE(sp_str_equal_cstr(got, "hi <!fmt: unterminated placeholder>"));
}

UTEST(sp_fmt_v, err_dynamic_fill_wrong_kind) {
  sp_str_t got = sp_fmt("{:$^5}", _sp_fmt_f64(1.0), _sp_fmt_int(42));
  EXPECT_TRUE(sp_str_equal_cstr(got, "<!fmt: dynamic fill expects int>"));
}

UTEST(sp_fmt_v, err_dynamic_width_wrong_kind) {
  sp_str_t got = sp_fmt("{:$}", _sp_fmt_cstr("oops"), _sp_fmt_int(42));
  EXPECT_TRUE(sp_str_equal_cstr(got, "<!fmt: dynamic width expects int>"));
}

UTEST(sp_fmt_v, err_dynamic_precision_wrong_kind) {
  sp_str_t got = sp_fmt("{:.$}", _sp_fmt_f64(3.0), _sp_fmt_f64(3.14));
  EXPECT_TRUE(sp_str_equal_cstr(got, "<!fmt: dynamic precision expects int>"));
}

UTEST(sp_fmt_v, err_stops_subsequent_placeholders) {
  sp_fmt_directive_reset();
  sp_str_t str = sp_zero();
  sp_err_t err = sp_fmt_e(&str, "{} {:nope} {}", _sp_fmt_int(1), _sp_fmt_int(2), _sp_fmt_int(3));
  EXPECT_TRUE(SP_ERR_FMT_UNKNOWN_DIRECTIVE);
}

UTEST(sp_fmt_v, str_precision_truncates) {
  sp_str_t got = sp_fmt("{:.3}", _sp_fmt_cstr("hello"));
  EXPECT_TRUE(sp_str_equal_cstr(got, "hel"));
}

UTEST(sp_fmt_v, str_precision_longer_than_string) {
  sp_str_t got = sp_fmt("{:.10}", _sp_fmt_cstr("hi"));
  EXPECT_TRUE(sp_str_equal_cstr(got, "hi"));
}

UTEST(sp_fmt_v, str_dynamic_precision_truncates) {
  sp_str_t got = sp_fmt("{:.$}", _sp_fmt_int(2), _sp_fmt_cstr("hello"));
  EXPECT_TRUE(sp_str_equal_cstr(got, "he"));
}

UTEST(sp_fmt_v, str_precision_with_width) {
  sp_str_t got = sp_fmt("[{:>6.3}]", _sp_fmt_cstr("hello"));
  EXPECT_TRUE(sp_str_equal_cstr(got, "[   hel]"));
}

UTEST(sp_fmt_v, f64_precision_zero_means_zero) {
  sp_str_t got = sp_fmt("{:.0}", _sp_fmt_f64(3.7));
  EXPECT_TRUE(sp_str_equal_cstr(got, "4"));
}

UTEST(sp_fmt_v, f64_dynamic_precision_zero) {
  sp_str_t got = sp_fmt("{:.$}", _sp_fmt_int(0), _sp_fmt_f64(3.7));
  EXPECT_TRUE(sp_str_equal_cstr(got, "4"));
}

UTEST(sp_fmt_v, f64_no_precision_defaults_to_six) {
  sp_str_t got = sp_fmt("{}", _sp_fmt_f64(1.5));
  EXPECT_TRUE(sp_str_equal_cstr(got, "1.500000"));
}

UTEST(sp_fmt_v, width_clamped_literal) {
  sp_str_t got = sp_fmt("{:99999}", _sp_fmt_cstr("x"));
  EXPECT_EQ(got.len, _SP_FMT_WIDTH_MAX);
}

UTEST(sp_fmt_v, width_clamped_dynamic_huge) {
  sp_str_t got = sp_fmt("{:$}", _sp_fmt_u64(999999999ULL), _sp_fmt_cstr("x"));
  EXPECT_EQ(got.len, _SP_FMT_WIDTH_MAX);
}

UTEST(sp_fmt_v, width_clamped_dynamic_negative) {
  sp_str_t got = sp_fmt("{:$}", _sp_fmt_int(-5), _sp_fmt_cstr("hi"));
  EXPECT_TRUE(sp_str_equal_cstr(got, "hi"));
}

static void _test_render_u64_only(sp_str_builder_t* b, _sp_fmt_arg_t* arg) {
  _sp_fmt_write_u64(b, arg->u);
}
...
UTEST(sp_fmt_directive, kinds_unset_accepts_all) {
  ...
  sp_str_t c = sp_fmt("{:any}", _sp_fmt_f64(2.0));
