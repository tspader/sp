...
void header(const c8* key, const c8* value) {
  sp_log_a("[{.fg brightcyan}] -> {.fg brightgreen}", sp_fmt_cstr(key), sp_fmt_cstr(value));
}
...
s32 run(s32 num_args, const c8** args) {
  ...
  sp_ht_for_kv(hts.key, it) {
    sp_log_a("[{}, {}] -> {} {.fg brightblack}", sp_fmt_int(it.key->foo), sp_fmt_uint(it.key->bar), sp_fmt_cstr(*it.val), sp_fmt_ptr(it.val));
  }
