...
.PHONY: all clean test test-nostdlib test-nostdlib-x64 test-nostdlib-arm64 test-builtin test-builtin-x64 test-builtin-arm64 test-jit test-jit-x64 test-embed test-embed-x64 test-embed-arm64

all: test

test: test-nostdlib test-builtin test-jit test-embed
...
test-embed: test-embed-x64 test-embed-arm64

test-embed-x64: $(BUILD_DIR)/embed_x64
ifeq ($(UNAME_S),Darwin)
ifndef HAS_DOCKER
	$(error docker not running)
endif
endif
	$(RUN_X64) ./$(BUILD_DIR)/embed_x64 $(BUILD_DIR)/embed_test.o Makefile sp.h
	readelf -s $(BUILD_DIR)/embed_test.o

$(BUILD_DIR)/embed_x64: example/freestanding/embed/embed.c sp.h | $(BUILD_DIR)
	$(CC_X64) $(CFLAGS_SP_ELF) $(CFLAGS_DEBUG) -I. -o $@ example/freestanding/embed/embed.c

test-embed-arm64: $(BUILD_DIR)/embed_arm64
ifeq ($(UNAME_S),Darwin)
ifndef HAS_DOCKER
	$(error docker not running)
endif
...
ifndef HAS_QEMU_ARM
	...
	$(RUN_ARM64) ./$(BUILD_DIR)/embed_arm64 $(BUILD_DIR)/embed_test_arm64.o Makefile sp.h
	readelf -s $(BUILD_DIR)/embed_test_arm64.o

$(BUILD_DIR)/embed_arm64: example/freestanding/embed/embed.c sp.h | $(BUILD_DIR)
	$(CC_ARM64) $(CFLAGS_SP_ELF) $(CFLAGS_DEBUG) -I. -o $@ example/freestanding/embed/embed.c
