...
void sp_cli_write_help(sp_term_t* term, sp_cli_t* cli) {
  ...
  if (sp_da_size(cli->commands)) {
    ...
    sp_cli_write_heading(term, theme.heading, "commands");
    sp_da_for(cli->commands, it) {
  ...
  if (cli->num_own_opts) {
    ...
    sp_cli_write_heading(term, theme.heading, "options");
    sp_for(it, cli->num_own_opts) {
  ...
  if (sp_da_size(cli->args)) {
    ...
    sp_cli_write_heading(term, theme.heading, "arguments");
    sp_da_for(cli->args, it) {
      sp_cli_arg_t* arg = &cli->args[it];
      if (arg->arity == SP_CLI_ARG_REQUIRED) {
        sp_cli_write_label_hint(term, theme.label, sp_zero_s(sp_cli_theme_entry_t), labels[it], "required", sp_cstr_as_str(arg->summary), width);
      }
      else {
  ...
  if (cli->num_own_env) {
    ...
    sp_cli_write_heading(term, theme.heading, "environment");
    sp_for(it, cli->num_own_env) {
      sp_cli_env_t* var = &cli->env[it];
      if (var->required) {
        sp_cli_write_label_hint(term, theme.label, sp_zero_s(sp_cli_theme_entry_t), sp_cstr_as_str(var->name), "required", sp_cstr_as_str(var->summary), width);
      }
      else {
