/*
  ...
  per platform (see sp_tls_native_backend):
    ...
    - SP_TLS_BACKEND_ANCHORS    (Windows, Linux)
        ...
        trust anchors. mbedTLS performs verification. Windows enumerates the ROOT system store and
        filters by server-auth EKU; Linux reads SSL_CERT_FILE if set, else the distro's PEM bundle.
