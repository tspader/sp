...
SP_INLINE s32 sp_atomic_s32_load(sp_atomic_s32_t* value, sp_atomic_order_t order) {
  ...
  #elif defined(SP_MSVC)
    #if defined(SP_AMD64)
      s32 result = __iso_volatile_load32((volatile s32*)value);
      if (order != SP_ATOMIC_RELAXED) _ReadWriteBarrier();
      return result;
    #else
      (void)order;
      return (s32)_InterlockedOr((volatile long*)value, 0);
    #endif
  #elif defined(SP_GNUC)
    (void)order;
    __sync_synchronize();
    s32 result = *(volatile s32*)value;
    __sync_synchronize();
    return result;
  #else
    (void)order;
    ...
    __atomic_store_n(value, desired, sp_atomic_order_gnu_store(order));
      ...
      switch (order) {
        case SP_ATOMIC_RELAXED: {
          __iso_volatile_store32((volatile s32*)value, desired);
          break;
        }
...
SP_INLINE s32 sp_atomic_s32_exchange(sp_atomic_s32_t* value, s32 desired, sp_atomic_order_t order) {
  ...
  #elif defined(SP_MSVC)
    (void)order;
    return (s32)_InterlockedExchange((volatile long*)value, (long)desired);
  #elif defined(SP_GNUC)
    (void)order;
    s32 old = __sync_lock_test_and_set(value, desired);
    __sync_synchronize();
    return old;
  #else
    (void)order;
...
SP_INLINE s32 sp_atomic_s32_add(sp_atomic_s32_t* value, s32 add, sp_atomic_order_t order) {
  #if defined(SP_ATOMIC_GNU)
    return __atomic_fetch_add(value, add, sp_atomic_order_gnu(order));
  #elif defined(SP_MSVC)
    (void)order;
    return (s32)_InterlockedExchangeAdd((volatile long*)value, (long)add);
  #elif defined(SP_GNUC)
    (void)order;
...
SP_INLINE bool sp_atomic_s32_cas(sp_atomic_s32_t* value, s32 current, s32 desired, sp_atomic_order_t order) {
  ...
  #elif defined(SP_MSVC)
    (void)order;
    return _InterlockedCompareExchange((volatile long*)value, (long)desired, (long)current) == (long)current;
  #elif defined(SP_GNUC)
    (void)order;
...
SP_INLINE s64 sp_atomic_s64_load(sp_atomic_s64_t* value, sp_atomic_order_t order) {
  #if defined(SP_ATOMIC_GNU)
    return __atomic_load_n(value, sp_atomic_order_gnu_load(order));
  #elif defined(SP_MSVC)
    #if defined(SP_AMD64)
      s64 result = __iso_volatile_load64((volatile s64*)value);
      if (order != SP_ATOMIC_RELAXED) _ReadWriteBarrier();
      return result;
    #else
      (void)order;
      return (s64)_InterlockedOr64((volatile __int64*)value, 0);
    #endif
  #elif defined(SP_GNUC)
    (void)order;
    __sync_synchronize();
    s64 result = *(volatile s64*)value;
    __sync_synchronize();
    return result;
  #else
    (void)order;
...
SP_INLINE void sp_atomic_s64_store(sp_atomic_s64_t* value, s64 desired, sp_atomic_order_t order) {
  #if defined(SP_ATOMIC_GNU)
    __atomic_store_n(value, desired, sp_atomic_order_gnu_store(order));
  #elif defined(SP_MSVC)
    #if defined(SP_AMD64)
      switch (order) {
        case SP_ATOMIC_RELAXED: {
          __iso_volatile_store64((volatile s64*)value, desired);
          break;
        }
        case SP_ATOMIC_RELEASE: {
          _ReadWriteBarrier();
...
SP_INLINE s64 sp_atomic_s64_exchange(sp_atomic_s64_t* value, s64 desired, sp_atomic_order_t order) {
  #if defined(SP_ATOMIC_GNU)
    return __atomic_exchange_n(value, desired, sp_atomic_order_gnu(order));
  #elif defined(SP_MSVC)
    (void)order;
    return (s64)_InterlockedExchange64((volatile __int64*)value, (__int64)desired);
  #elif defined(SP_GNUC)
    (void)order;
    s64 old = __sync_lock_test_and_set(value, desired);
    __sync_synchronize();
    return old;
  #else
    (void)order;
...
SP_INLINE s64 sp_atomic_s64_add(sp_atomic_s64_t* value, s64 add, sp_atomic_order_t order) {
  #if defined(SP_ATOMIC_GNU)
    return __atomic_fetch_add(value, add, sp_atomic_order_gnu(order));
  #elif defined(SP_MSVC)
    (void)order;
    return (s64)_InterlockedExchangeAdd64((volatile __int64*)value, (__int64)add);
  #elif defined(SP_GNUC)
    (void)order;
    return __sync_fetch_and_add(value, add);
  #else
    ...
    sp_spin_lock(lock);
    s64 old = *value;
    *value = (s64)((u64)old + (u64)add);
    sp_spin_unlock(lock);
    return old;
  #endif
}

SP_INLINE bool sp_atomic_s64_cas(sp_atomic_s64_t* value, s64 current, s64 desired, sp_atomic_order_t order) {
  #if defined(SP_ATOMIC_GNU)
    return __atomic_compare_exchange_n(value, &current, desired, false, sp_atomic_order_gnu(order), sp_atomic_order_gnu_fail(order));
  #elif defined(SP_MSVC)
    (void)order;
    return _InterlockedCompareExchange64((volatile __int64*)value, (__int64)desired, (__int64)current) == (__int64)current;
  #elif defined(SP_GNUC)
    (void)order;
    return __sync_bool_compare_and_swap(value, current, desired);
  #else
...
SP_INLINE bool sp_atomic_ptr_cas(sp_atomic_ptr_t* value, void* current, void* desired, sp_atomic_order_t order) {
  #if defined(SP_ATOMIC_GNU)
    return __atomic_compare_exchange_n(value, &current, desired, false, sp_atomic_order_gnu(order), sp_atomic_order_gnu_fail(order));
  #elif defined(SP_MSVC)
    (void)order;
...
#define SP_NT_STATUS_NOT_A_REPARSE_POINT   ((sp_nt_status_t)0xC0000275)
...
#define SP_NT_FSCTL_GET_REPARSE_POINT 0x000900A8
...
sp_err_t sp_sys_is_supported(u32 what) {
  return sp_atomic_s32_load(&sp_rt.unsupported[what], SP_ATOMIC_RELAXED) ? SP_ERR_SYS_UNSUPPORTED : SP_OK;
}

void sp_sys_mark_supported(u32 what) {
  sp_atomic_s32_store(&sp_rt.unsupported[what], 0, SP_ATOMIC_RELAXED);
}

sp_err_t sp_sys_mark_unsupported(u32 what) {
  sp_atomic_s32_store(&sp_rt.unsupported[what], 1, SP_ATOMIC_RELAXED);
...
// The inverse. Take:
// - Paths in the NT namespace (\??\), like in a reparse point
// - Paths in the Win32 verbatim namespace (\\?\), like GetFinalPathNameByHandle
//
// And turn them into the DOS spelling that Win32 accepts:
//
//   \??\C:\A\B                C:\A\B
...
//
// Works in place and returns a view
SP_PRIVATE sp_wide_str_t sp_sys_dos_from_nt_wtf16(u16* nt, u32 len) {
  sp_wide_str_t out = { .data = nt, .len = len };
...
SP_PRIVATE sp_err_t sp_sys_err_from_nt(sp_nt_status_t status) {
  ...
  switch (status) {
    ...
    case SP_NT_STATUS_NOT_A_REPARSE_POINT:    return SP_ERR_SYS_INVALID;
...
SP_PRIVATE sp_err_t sp_sys_nt_set_name_info(sp_sys_fd_t from_fd, sp_str_t from, sp_sys_fd_t to_fd, sp_str_t to, u32 access, u32 flags, u32 info_class, u32 legacy_info_class) {
  ...
  *info = sp_zero_s(sp_nt_file_rename_information_t);
  ...
  // NT reports a target whose parent exists but is not a directory as
  // INVALID_PARAMETER, for both the Ex and legacy classes and for links. A
  // missing parent is PATH_NOT_FOUND. This is NT's ENOTDIR for these two
  // primitives; elsewhere INVALID_PARAMETER still means a bug on our side.
  if (status == SP_NT_STATUS_INVALID_PARAMETER) return SP_ERR_SYS_NOT_DIR;
  return sp_sys_err_from_nt(status);
}
...
// SP_SYS_READLINK //
/////////////////////
sp_err_t sp_sys_readlink_p(sp_sys_fd_t fd, const c8* path, u32 len, c8* buf, u64 size, u64* target_len) {
  *target_len = 0;
  ...
  sp_nt_status_t status = sp_sys_nt_open(
    ...
    FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
    SP_NT_FILE_OPEN,
    SP_NT_FILE_SYNCHRONOUS_IO_NONALERT | SP_NT_FILE_OPEN_REPARSE_POINT | SP_NT_FILE_OPEN_FOR_BACKUP_INTENT,
    0,
    &handle
  );
  if (!SP_NT_SUCCESS(status)) return sp_sys_err_from_nt(status);
  ...
  status = SP_NT(NtFsControlFile)(
    (void*)handle, SP_NULLPTR, SP_NULLPTR, SP_NULLPTR, &iosb,
    SP_NT_FSCTL_GET_REPARSE_POINT, SP_NULLPTR, 0, rdb_buf, sizeof(rdb_buf)
  );
  sp_sys_nt_close(handle);
  if (!SP_NT_SUCCESS(status)) return sp_sys_err_from_nt(status);

  sp_nt_symlink_reparse_buffer_t* rdb = (sp_nt_symlink_reparse_buffer_t*)rdb_buf;
  if (rdb->ReparseTag != SP_NT_IO_REPARSE_TAG_SYMLINK) return SP_ERR_SYS_INVALID;

  u16* names = (u16*)(rdb_buf + sizeof(sp_nt_symlink_reparse_buffer_t));
  u16* name = names + rdb->SubstituteNameOffset / sizeof(u16);
  u32 name_len = rdb->SubstituteNameLength / (u32)sizeof(u16);
  sp_wide_str_t wide = { .data = name, .len = name_len };
  if (!(rdb->Flags & SP_NT_SYMLINK_FLAG_RELATIVE)) {
    wide = sp_sys_dos_from_nt_wtf16(name, name_len);
  }

  SP_ALIGNED c8 utf8_buf [SP_NT_MAX_REPARSE_DATA * 3 / 2];
  sp_mem_fixed_t fixed = sp_mem_fixed(utf8_buf, sizeof(utf8_buf));
  sp_str_t utf8 = sp_zero;
  sp_try(sp_wtf16_to_wtf8(sp_mem_fixed_as_allocator(&fixed), wide, &utf8));
  if (utf8.len >= size) return SP_ERR_SYS_NAME_TOO_LONG;
  ...
  *target_len = utf8.len;
  ...
  sp_try(sp_sys_posix_path(path, len, cpath));
  s64 rc = sp_syscall(SP_SYSCALL_NUM_READLINKAT, fd, cpath, buf, size);
  if (sp_sys_is_err(rc)) return sp_sys_err_from_errno(-rc);
  if ((u64)rc == size) return SP_ERR_SYS_NAME_TOO_LONG;
  *target_len = (u64)rc;
  return SP_OK;
  ...
  c8 cpath [SP_PATH_MAX];
  sp_try(sp_sys_posix_path(path, len, cpath));
  s64 rc = readlinkat((int)fd, cpath, buf, (size_t)size);
  if (rc < 0) return sp_sys_err_from_errno(errno);
  if ((u64)rc == size) return SP_ERR_SYS_NAME_TOO_LONG;
  *target_len = (u64)rc;
  return SP_OK;
...
sp_err_t sp_sys_readlink_s(sp_sys_fd_t fd, sp_str_t path, c8* buf, u64 size, sp_str_t* target) {
  u64 len = 0;
  sp_err_t err = sp_sys_readlink(fd, path.data, path.len, buf, size, &len);
  *target = sp_str(buf, (u32)len);
  return err;
}
...
sp_err_t sp_sys_set_times_p(sp_sys_fd_t fd, const c8* path, u32 len, sp_sys_timespec_t atime, sp_sys_timespec_t mtime) {
  (void)fd; (void)path; (void)len; (void)atime; (void)mtime;
  return SP_ERR_SYS_UNSUPPORTED;
}
...
sp_err_t sp_io_file_reader_discard(sp_io_reader_t* reader, u64 n, u64* discarded) {
  sp_io_file_reader_t* r = (sp_io_file_reader_t*)reader;
  u64 size = 0;
  sp_err_t err = sp_io_file_reader_size(r, &size);
  if (err) {
    if (discarded) *discarded = 0;
    return err;
  }
  u64 remaining = r->pos < size ? size - r->pos : 0;
...
sp_err_t sp_io_file_reader_seek(sp_io_file_reader_t* r, s64 offset, sp_io_whence_t whence, s64* position) {
  ...
  switch (whence) {
    ...
    case SP_IO_SEEK_END: {
      u64 size = 0;
      err = sp_io_file_reader_size(r, &size);
      if (err) {
        pos = -1;
        goto done;
      }
...
sp_err_t sp_io_file_reader_size(sp_io_file_reader_t* r, u64* size) {
  if (!r->size_known) return sp_io_file_reader_size_force(r, size);
  if (size) *size = r->size;
  return SP_OK;
}
...
sp_err_t sp_io_file_writer_write(sp_io_writer_t* writer, const void* ptr, u64 size, u64* bytes_written) {
  ...
  w->pos += num_bytes;
  if (w->size_known && w->pos > w->size) w->size = w->pos;
...
sp_err_t sp_io_file_writer_size(sp_io_file_writer_t* w, u64* size) {
  sp_assert(w);
  if (!w->size_known) return sp_io_file_writer_size_force(w, size);
  if (size) *size = w->size;
  return SP_OK;
}
...
#define SP_IO_PAD_CHUNK 4096

sp_err_t sp_io_pad(sp_io_writer_t* writer, u64 size, u64* bytes_written) {
  static const u8 zeros [SP_IO_PAD_CHUNK] = sp_zero;
  ...
  while (total < size) {
    u64 chunk_written = 0;
    result = sp_io_write_all(writer, zeros, sp_min(size - total, sizeof(zeros)), &chunk_written);
    total += chunk_written;
    if (result) break;
  }
...
SP_PRIVATE sp_err_t sp_fs_mkdir_at(sp_sys_fd_t dir, sp_str_t path) {
  ...
  sp_sys_file_meta_t st = sp_zero;
  if (sp_sys_get_path_metadata_s(dir, path, &st)) return err;
...
SP_PRIVATE sp_err_t sp_fs_atomic_create(sp_fs_atomic_t* af, sp_sys_fd_t dir, sp_str_t path, sp_str_t temp_dir) {
  *af = sp_zero_s(sp_fs_atomic_t);
  ...
  err = sp_io_file_writer_from_fd(&af->writer, fd, SP_IO_CLOSE_MODE_AUTO);
  if (err) {
...
SP_PRIVATE sp_err_t sp_fs_atomic_publish(sp_fs_atomic_t* af, sp_fs_atomic_mode_t mode) {
  switch (mode) {
    case SP_FS_ATOMIC_REPLACE:
      return sp_sys_rename_s(af->dir, af->temp, af->dir, af->path);
    case SP_FS_ATOMIC_EXCLUSIVE: {
      sp_err_t err = sp_sys_link_s(af->dir, af->temp, af->dir, af->path);
...
sp_err_t sp_fs_atomic_abort(sp_fs_atomic_t* af) {
  ...
  sp_err_t err = sp_sys_unlink_s(af->dir, af->temp);
  af->temp = sp_zero_s(sp_str_t);
  return err;
}

SP_PRIVATE sp_err_t sp_fs_atomic_write(sp_fs_atomic_t* af, const void* data, u64 size) {
  sp_err_t err = sp_io_write_all(sp_fs_atomic_writer(af), data, size, SP_NULLPTR);
  if (err) {
