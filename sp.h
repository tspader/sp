...
void sp_context_ensure() {
  sp_context_t* ctx = sp_context_get();
  if (!ctx || !ctx->allocator.on_alloc) sp_init_default();
}
...
sp_str_t sp_str_concat(sp_str_t a, sp_str_t b) {
  ...
  sp_context_push_allocator(ctx->temp_allocator_interface);

  // Allocate with temp allocator
  u32 total_len = a.len + b.len;
  c8* buffer = (c8*)sp_alloc(total_len);
  sp_os_copy_memory(a.data, buffer, a.len);
  sp_os_copy_memory(b.data, buffer + a.len, b.len);
  sp_str_t temp_result = SP_STR(buffer, total_len);

  // Pop temp allocator context
  sp_context_pop();

  // Copy result with user allocator
  sp_str_t result = sp_str_copy(temp_result);

  // Restore watermark
  sp_bump_allocator_pop(&ctx->temp_allocator, mark);
...
sp_str_t sp_str_pad(sp_str_t str, u32 n) {
  ...
  // Allocate with temp allocator
  c8* buffer = (c8*)sp_alloc(n);
  sp_os_copy_memory(str.data, buffer, str.len);
  for (u32 i = 0; i < delta; i++) {
    buffer[str.len + i] = ' ';
  }
  sp_str_t temp_result = SP_STR(buffer, n);
...
sp_str_t sp_str_replace_c8(sp_str_t str, c8 from, c8 to) {
  ...
  sp_str_t temp_result = SP_STR(buffer, str.len);
...
sp_ps_status_t sp_ps_poll(sp_ps_t* ps, u32 timeout_ms) {
  ...
  do {
    ...
    if (wait_result == 0) {
      ...
      result.state = SP_PROC_STATE_DONE;
